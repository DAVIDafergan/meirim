      ...
      {departments.map((dept) => {
        ...
        return (
          <motion.div
            key={dept.slug}
            variants={cardItem}
            className="flex h-full flex-col items-center gap-3 bg-background p-7 text-center"
          >
            {Icon && (
            ...
            <h3 className="font-display text-base font-bold text-foreground">{dept.name}</h3>
            <p className="line-clamp-3 flex-1 text-sm text-foreground-muted">{dept.summary}</p>
            <div className="mt-2 flex w-full flex-col gap-2">
              <Link href={`/departments/${dept.slug}`} className={`px-4 py-2 text-sm ${outlineButton}`}>
                לפרטים
              </Link>
              <a
                href={nedarimPlusUrl({
